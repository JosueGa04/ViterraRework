 ...
 */
export async function fetchTokkoUserRow(client: SupabaseClient, userId: string, email?: string) {
  const byId = await client.from("tokko_users").select("*").eq("id", userId).maybeSingle();
  if (!byId.error && byId.data) return byId;

  const resolvedEmail = email || (await client.auth.getUser()).data.user?.email?.trim().toLowerCase();
  if (!resolvedEmail) return byId;

  return client.from("tokko_users").select("*").ilike("email", resolvedEmail).maybeSingle();
}
